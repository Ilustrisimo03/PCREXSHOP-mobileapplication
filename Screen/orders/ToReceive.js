...
const ToReceive = ({ navigation }) => {
    ...
    const handleConfirmReception = (orderId) => {
        Alert.alert(
            "Confirm Receipt",
            "By confirming, you agree that you have received the order in good condition.",
            [
                { text: "Cancel", style: "cancel" },
                { text: "Yes, I've Received It", onPress: () => updateOrderStatus(orderId, 'To Review') }
            ]
        );
    };
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={styles.emptyContainer}>
                        <Icon name="truck-delivery-outline" size={70} color="#CCC" />
                        <Text style={styles.emptyText}>No orders are on their way.</Text>
                    </View>
                }
            />
        </SafeAreaView>
...
const styles = StyleSheet.create({
    ...
    orderStatus: { fontSize: 13, fontWeight: 'bold', color: '#2ECC71' },
